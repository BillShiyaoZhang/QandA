...
状态：设计提案，尚未实现。更新日期：2026-09-06。

输入：`docs/intent.md`，以及补充需求「访客可以提问、提交生成的答案」「回答之后可以追问，多个追问形成分支，需要管理与可视化」。原文后续增加的「分析并确定不同问题、不同分支之间的关联，将它们连接起来」也纳入本设计。目前项目没有应用代码，也尚未初始化 Git 仓库；下文的目录、命令和页面都是拟议设计。
...
`generation` 记录供应商、渠道、原样模型显示名、请求的模型 ID、响应返回的模型 ID、明确公开的版本标识、生成时间及时间精度。未知字段用 `null`，并允许描述缺失原因。
...
以下为熟悉 TypeScript 的单人开发粗估，包含基本验证，不含大规模内容整理和用户反馈等待时间。当前没有代码基础，实际工作量需在第一阶段后调整。
...
实际实现时，为图不变量、历史不可变、投稿快照、关系状态与上下文组装编写测试；页面做少量覆盖投稿入口、深链接、移动端、比较和关联跳转的端到端检查。当前阶段只有设计文档，不宣称这些验收已完成。
