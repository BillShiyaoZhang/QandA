...
更新：2026-09-06。面向文本问答档案与自动归集，普通投稿无需前置人工审批，监督与更正留待事后。不包括实时模型生成、站内账号或匿名投稿后端。
...
- `src/lib/issue-intake.ts`、`scripts/collect-issues.ts`：四类投稿自动收录、每 Issue 幂等、状态反馈与开放投稿补扫。
...
| 分支阅读 | `/questions/:id/?focus=:node` 默认分支图／树目录与同页阅读；旧 `/explore/:root/` 共用组件。按节点加载正文，手机默认显示分支，节点可分享、定位与前进后退 |
| 路径复制 | 答案页面及树阅读区：复制精确祖先路径；排除兄弟答案和横向关联，撤回时标记不完整 |
| 并排比较 | `/compare/?left=:id&right=:id`：同题跨修订比较、条件差异、手机 A/B 切换 |
| 横向关联 | 修订或答案页：理由、片段、社区提出者或人工确认者、最多 20 邻居的局部 SVG 图，普通链接可遍历更多关系 |
| 投稿 | `/contribute/`：提示公开性、GitHub 账号要求；只把模板和短 ID 预填入 GitHub 链接 |
| 内容维护 | CLI 导入、审阅、修订、注释、候选确认/驳回、归档/重新开放、撤回 |

默认工作区的使用案例和取舍见 [用户体验梳理](experience.md)。树把问题、答案与追问连成可点击的结构；单个当前修订与问题共用显示位置，多版本和撤回修订保持独立。修订标签使用顺序，答案用来源和摘要识别。默认最多渲染 100 个节点，深层节点及祖先链保持可定位。阅读区提供版本切换、答案排序／比较、投稿、注释与关联线索；详情页保留完整记录。关系图的实线表示回答或追问路径，虚线表示横向关联；只有 `supports` 表示单向支持。横向关联不改变父答案，也不加入上下文。
...
导入复用暂存草稿与事务机制：检查目标是否可见、是否归档以及已存在目标文件是否仍与基准一致，再在完整暂存内容库上校验。普通 Issue 由 Actions 自动完成这一步；本地人工入口保留给事后维护及特殊导入。

通过后先写事务记录，把旧内容目录移到备份，再切换新目录。成功导入的来源收据和内容在同一次替换中落盘。本地缓存丢失或最后状态写入中断，可以由收据恢复；重复导入不会产生第二份答案。

恢复依据当前目录、备份、事务记录和已提交收据/内容哈希判断。无法判断时保留现场。Actions 归集通过全局队列串行执行，普通推送遇到 main 并发变化时失败而不覆盖，后续补扫重新处理。人工维护仍通过 Git 分支、PR 和合并校验协调。
...
仓库已由所有者公开，投稿模板和 GitHub Pages 已配置。自动化无付费模型依赖，使用公开仓库标准 runner 与短期 `GITHUB_TOKEN`；token 提交后显式调用 Pages 发布，避免依赖不会触发的 push 事件。运行细节及恢复方式见 [自动化文档](automation.md)。
