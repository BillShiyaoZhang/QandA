...
已实现静态网站、内容校验、访客投稿表单、自动归集和事后维护工具。公开投稿通过 GitHub Issue Forms 进行，需要 GitHub 账号；只需写问题或回答，来源补充选填，位置自动带入，公开确认只做一次；通过格式及引用检查后自动收录、发布，无需维护者逐条审批。本项目不自动调用模型，不把收录视为事实核验。

- [设计与可行性分析](docs/design.md)
- [使用案例与用户体验](docs/experience.md)
- [用户信息清单与提交简化](docs/submission-experience.md)
