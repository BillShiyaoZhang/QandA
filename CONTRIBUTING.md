...
可以只提问，也可以写自己的回答或分享已有 AI 回答，不必重新生成。问题只写一遍，第一行会用作展示标题。模型、日期、链接与背景统一放在“来源补充（选填）”，不知道就留空；网站自动填入投稿位置，无需查找内部编号。模型显示名称来自提交者声明，收录不证明身份或事实正确。问题、答案和引用片段应保持原意，注明已知来源；提交表示允许项目存储、展示你的投稿及必要的格式整理，不替代第三方内容的授权。

追问应明确针对一份答案。修改同一个问题的实际提问属于修订；在另一条上下文中提出相同文字仍是另一个问题。对内容的更正可以通过 Issue 说明目标链接及证据，由维护者添加注释。

字段取舍和具体路径见[用户信息清单与提交简化](docs/submission-experience.md)。
...
「已收录」表示内容和来源收据已经保存，「已上线」取决于后续 Pages 发布成功，通常需要几分钟。发布成功后机器人更新原有回复，并自动把完成的投稿关闭；失败、暂停或正文已改动的投稿保留开放。关闭仅表示处理完成，不代表事实核验。失败时旧站点继续可读，每 6 小时的补扫会重试；维护者也可从 Actions 手动运行 `Collect issue submissions`。操作说明见 [自动化文档](docs/automation.md)。
...
日常投稿可把生成规则原样写在“来源补充（选填）”，作为公开注释保存。若需要结构化比较，旧版表单「生成规则（选填）」以及特殊导入的 `generation_protocol` 仍可使用。维护者也可通过文件的完整 `generation` 对象导入已知的供应商、渠道、请求/返回模型、时间精度、协议、参数、工具及结束状态；结构见 `generationSchema`，未知字段为 `null` 或对应未知枚举。它仍属于提交者声明，不会升级成平台捕获。若与简要模型名称、日期或规则冲突，导入器要求先核对，不会静默覆盖。
