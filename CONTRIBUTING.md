...
从网站的「提个新问题」「提交本题答案」「追问这份回答」或「建议关联」入口进入 GitHub 表单。需要 GitHub 账号；提交后内容先出现在 Issue 中，维护者审核收录后才进入网站。公开仓库中的 Issue 在审核前也公开，请先删除私人信息、密钥和无权公开的内容。

提交已有答案即可，不必为了投稿重新生成。不知道模型版本、时间或上下文时选择未知。模型显示名称来自提交者声明，审核收录不证明身份或事实正确。问题、答案和引用片段应保持原意，注明已知来源；提交表示允许项目存储、展示你的投稿及必要的格式整理，不替代第三方内容的授权。

追问应明确针对一份答案。修改同一个问题的实际提问属于修订；在另一条上下文中提出相同文字仍是另一个问题。对内容的更正可以通过 Issue 说明目标链接及证据，由维护者添加注释。

## 维护者审核

在独立内容分支上操作，先同步 `main`。本地审核命令只更新内容文件，不会向 GitHub 发送评论、关闭 Issue 或自动发布网站。
...
导入依据「来源 URL + 更新时间 + 正文哈希」去重；成功审核的收据进入 `content/imports/`，在另一个工作副本重复导入也不会增加答案。源 Issue 编辑会成为新的来源快照，不能覆盖已收录正文。修改审核解释或重试已驳回的草稿时使用 `--refresh` 重新导入并审阅。
...
候选分数只用于排序，不能解释为可信度。自动候选只表示主题相关；支持或冲突应经人工关联投稿，并提供双方原文片段和理由。确认前重新校验端点版本及可见性。已驳回候选不会以同一版本反复出现；需要重新判断时新建人工关联，保留原决定。
