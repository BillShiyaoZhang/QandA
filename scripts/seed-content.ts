...
if (fs.existsSync(path.join(root, 'questions'))) {
  console.error('样本已存在；seed 不覆盖内容。使用空 CONTENT_DIR 可另建样本。');
  process.exit(1);
}
...
for (const [i, source, target, reason] of [
  ...
  const getHash = (id: string) =>
    id.startsWith('a-')
      ? JSON.parse(fs.readFileSync(path.join(root, `answers/${id}/meta.json`), 'utf8')).body_sha256
      : JSON.parse(
          fs.readFileSync(
...
console.log(`开发样例已写入 ${root}。所有示例均显式标识，不伪造模型版本或社区来源。`);
